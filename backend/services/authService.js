...
const register = async ({ name, email, password, isVet }) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, isVet,name }, JWTSECRET, { expiresIn: '1d' });
...
const login = async ({ email, password }) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, isVet: user.isVet,name:user.name }, JWTSECRET, { expiresIn: '1d' });
